...
- Next.js의 Image 컴포넌트는 초보자도 이미지 최적화를 할 수 있게 도와주는 매우 강력한 내장 기능이다.
- 그러나 공식 문서상 설명이 애매한 부분이 다소 있기 때문에 이 기회에 제대로 정리 하고자 한다.

> Next.js 이미지 컴포넌트에 대해 얼마나 알고 있는지 알아보기위해 warm-up quiz를 준비했다.
> 이 포스트를 다 읽은 후에는 이 질문들에 대해서 모두 답할 수 있을 것이다.
...
- 개발자 도구 Network 탭 fast 3g 환경에서 31.77초 소요, 참고로 구글에서 권장하는 LCP 기준은 2.5초 이하!, 로컬호스트인 것을 감안하면 실제 서버에서는 더 느려진다.
...
- `size="100vw"` 하나 넣었다고 이미지 크기가 다시 한번 1/6이로 줄어듬
...
- 긜고 실제 dashboard에서는 지금까지 프로젝트에서 사용한 총 이미지 source의 개수를 확인 할 수 있다.
...
- 여기에서 말하는 source images란 개별 이미지 이미지을 말함.
...
<img width="500" alt="image" src="https://github.com/CS-TeamStudy/CS_Study_for_Interview/assets/87072568/3acd41dd-429b-4c10-b6b4-eb6c2cb795d8">
...
- 정적 이미지 설정시 자동으로 height와 width가 설정된 것을 확인 할 수 있다. 그러나 이는 웹페이지 상에서 보여질 width와 height가 아니라 원본의 width와 height다.
  ...
  - width를 조절한다고 이미지의 파일 사이즈(102kB)가 줄어들진 않는다.
  - 정적 이미지는 이미 이미지의 비율을 알고 있기 떄문에 width 또는 height 속성 둘중 하나만을 입력해도 된다.
  - width속성을 통해 로드 될 이미지의 크기를 미리 지정하여 CLS(cumulative layout shift)를 막을 수 있다.

  - 정리해보자면 width(or height) 속성의 역할은 CLS방지와 화면에 보여질 이미지의 width와 heght를 정의해주는 것이다.
...
- svg + sml 형식의 이미지가 blurDataURL 이미지이며 width가 8, 이미지 품질이 70%인 이미지다.
...
- 예상 되는 에러는??
  ...
  - 원격 외부 이미지의 비율을 모르기 떄문에 반드시 height 속성도 넣어줘야한다.
...
- 다양한 출처의 원격 외부 이미지는 width와 hegiht가 일정하지 않기 때문에 만약 일정한 크기의 이미지를 보여주고 싶다면 다음과 같은 문제가 발생할 수 있다.
...
- 그리고 첫번째 이미지는 width와 height 설정을 500px로 했지만 실제 이미지는 1:1 비율이 아니므로 height(500px)가 무시된다.
...
- 그러나 이렇게 하면 사진이 강제적으로 비율이 1:1(500px:500px)이 된다. 이는 css onject 속성을 cover로 해주면 비율을 유지한채 부모요소를 가득 채우게 된다. 경우에 따라 contain이나 다른 값을 사용 할 수 있다.
...
- srcset과 size에 관한 [내용](https://developer.mozilla.org/en-US/docs/Learn/HTML/Multimedia_and_embedding/Responsive_images)은 mdn에서 읽어보길 바랍니다.
...
- size값을 주지 않은 `<Image alt="cat" src={cat} />`의 경우와 다르게 <br/> `<Image alt="cat" src={cat} sizes="100vw" />`의 경우 next가 알아서 다양한 width에 해당하는 image srcset을 제공해준다.
- size 계산이 간단한 경우라면 쉽게 sizes 속성을 설정해줄 수 있다.

  - `<Image alt="cat" src={cat} width={500} />` 같은 경우 기기의 width가 500이하인 경우 width의 100%를 차지하고 기기의 width가 500보다 큰 경우 사진의 width는 500으로 일정하다. 이를 sizes속성에 적용하면 다음과 같다.
...
- 계산이 귀찮다면 이를 대신해주는 곳이 있다. https://ausi.github.io/respimagelint/
  ...
  - size값을 일부러 잘못 입력한 다음 북마크로 추가한 `Lint Images`를 누르면 된다.
  ...
  - 아래과 같이 복잡한 size를 설정해야하는 경우에 유용하다.
  ...
  export default async function Home() {
    return (
      ...
      // md(tailwind에서 설정한 break point) 이상이면 grid column이 3줄, sm이상이면 grid column이 2줄, 이하는 1줄
